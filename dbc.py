...
def define_config():
    ...
    # natural or not
    config.natural = True
    config.custom_video = False

    # obs model
    config.obs_model = 'dbc'
...
class Dreamer(tools.Module):
    ...
    def _train(self, data, log_images):
        with tf.GradientTape() as model_tape:
            ...
            post2, prior2 = self._dynamics.observe(embed2, data['action'])

            feat = self._dynamics.get_feat(post)
            feat2 = self._dynamics.get_feat(post2)

            reward_pred = self._reward(feat)
            reward_pred2 = self._reward(feat2)
            ...
            post_dist2 = self._dynamics.get_dist(post2)
            mae = tf.keras.losses.MeanAbsoluteError(reduction=tf.keras.losses.Reduction.NONE)
            

            z_dist = tf.math.reduce_mean(mae(embed,embed2), axis = 1)
            # r_dist = tfd.kl_divergence(reward_pred,reward_pred2)
            r_dist = mae(data['reward'],data['reward'][perm])


            # transition_dist = tfd.kl_divergence(post_dist, post_dist2)

            ## Wasserstein distance b/w state transition kernels
            transition_dist = tf.math.reduce_mean(tf.math.sqrt(tf.math.square(post['mean'] - post2['mean']) + tf.math.square(post['std'] - post2['std'])),axis=[1,2])
            

            bisim = r_dist + self._c.discount * transition_dist
            dbc_loss = tf.math.square(z_dist - bisim)
            # tf.print(z_dist)
            # tf.print(r_dist)
            # tf.print(transition_dist)
            # tf.print(dbc_loss)
            ...
            elif self._c.obs_model == 'dbc':
                likes.image = tf.reduce_mean(image_pred.log_prob(data['image']))
                likes.image1 = tf.reduce_mean(dbc_loss)
