...
class Dreamer(tools.Module):
    ...
    def _train(self, data, log_images):
        with tf.GradientTape() as model_tape:
            ...
            transition_dist = tf.math.reduce_mean(tf.math.sqrt(tf.math.square(post['mean'] - prior['mean']) + tf.math.square(post['std'] - prior['std'])),axis=[1,2])
            

            
            deepmdp_transition_loss = self._c.discount * transition_dist
            # tf.print(z_dist)
            # tf.print(r_dist)
            ...
            if self._c.obs_model == 'generative':
                ...
                likes.image = tf.reduce_mean(image_pred.log_prob(data['image']))
                likes.image1 = tf.reduce_mean(deepmdp_transition_loss)
